...
- **PPTX formula export**: when PPTX output contains formulas, verify that the chosen authoring route can create native equation objects or place verified rendered formula SVG/PNG assets. Do not use default text boxes, raw LaTeX, plain Unicode approximations, or placeholder equation strings as a fallback.
