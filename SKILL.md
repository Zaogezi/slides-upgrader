...
8. **Test the upgraded material**: read `references/test-standards.md`, then test the completed knowledge graph, materialized output, selected-output verification, and required deliverables. Run the Source Coverage Test to confirm every substantive source item from `extracts/<task-slug>.source.md` / `.json` was captured into `assets/<task-slug>.knowledge-graph.completed.md` and that nothing was dropped or summarized away during graph construction or completion. Record test results, preflight results, render evidence, and final checklist status in a quality report. Do not deliver if any blocker-level test fails.
...
- Include a source capture statement confirming that every substantive item recovered from the source in `extracts/<task-slug>.source.md` / `.json` was placed into `assets/<task-slug>.knowledge-graph.md` and survives in `.completed.md`, or list the specific source items not yet captured as a blocker.
...
- Step 3, knowledge graph construction: read `references/knowledge-graph-template.md`. It defines the Markdown template exported to `assets/<task-slug>.knowledge-graph.md`, the lossless source-capture requirement, and the Unassigned Source Content section used when a source item does not fit an existing field.
