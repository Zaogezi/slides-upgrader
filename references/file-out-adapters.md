...
All text artifacts created during materialization and verification must be UTF-8: coverage tables, rendering inventories, QA logs, generated SVG/HTML assets, quality reports, and copied Markdown/JSON artifacts. Do not use platform-default encoding or lossy replacement when reading or writing these files.

For PPTX output, the style preset is not a single-slide template to repeat mechanically. Use varied slide structures that match the learning function of each section: chapter overview, concept introduction, definition and symbols, derivation or algorithm, example walkthrough, interactive check, and chapter summary. A deck of six or more slides that is dominated by one slide layout or one repeated visual treatment fails visual verification unless the user explicitly requested a uniform template.
...
- Use multiple slide structures from the preset's preferred slide types. Do not repeat one layout or one visual template across the whole deck unless the user explicitly requested a uniform template.
...
- Generated text artifacts have passed UTF-8 auditing, including copied Markdown/JSON, coverage tables, QA logs, rendering inventories, SVG/HTML assets, and the quality report.
- PPTX output with six or more slides uses multiple slide layouts or visibly distinct structures appropriate to the learning flow; one-template decks are blocker-level unless explicitly requested.
- PPTX coverage and variety audit has no coverage or variety blockers when direct PPTX access is available.
...
- The quality report includes UTF-8 artifact audit path and status.
...
- The quality report includes PPTX coverage/variety audit path and status when PPTX output is produced.
